...
function publicAds(s, campaigns, clientTz) {
  ...
  if (!s.connected) {
    out.note = 'No ad account is connected for this business yet, so ad spend, ad results and return on ad spend are not available. Facebook paid views (from boosts and ads) are still available from the paid/organic split.';
    return out;
  }
  ...
  if (!s.totals) {
    if (byCur.length > 1) {
      ...
      out.totals = null;
      out.note = 'Meta shows no ad spend for this business in this period.';
    }
  } else {
...
const TOOLS = {
  ...
  get_influencer_posts: {
    description: 'Posts that influencers and other creators published about the business on Instagram (collabs, and posts that tag it): each creator\'s @handle and followers, the post\'s views, likes and comments, engagement rate, how its views compare with the business\'s usual reel, views gained since it was first read, the cost per 1,000 views when a cost was recorded, and its latest comments. The numbers are the public ones as Instagram shows them on each creator\'s post (a reel also shared to Facebook includes its Facebook views and likes, split out). Omit start/end for every recorded post, or pass a period for the posts published in it. Use for "how did the influencer do", "which creator brought the most views", "our collabs", "who posted about us".',
...
// influencer answer), so an answer about the business's own posts never drifts into creators' posts.
